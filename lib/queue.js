...
function Queue(name, opts) {
  ...
  var messageListener = function(channel, message) {
    if(channel !== self.getPrefix()) {
      return;
    }

    if(message === 'enqueue' && self.state === Queue.STARTED) {
      self.launchJob();
    }

    if(message === 'start' && self.state === Queue.STOPPED) {
      self.start();
    }

    if(message === 'stop' && self.state === Queue.STARTED) {
      self.stop();
    }
  };
  ...
  if(!this.sconn.connected) {
    this.sconn.once('ready', function() {
      self.subscribe(errorEmitter);
    });
  }
  else {
    this.subscribe(errorEmitter);
  }

  this.once('remove', function() {
    self.unsubscribe(errorEmitter);
...
Queue.prototype.subscribe = function subscribe(cb) {
  this.subscribed = true;
  this.sconn.subscribe(this.getPrefix(), cb);
};

Queue.prototype.unsubscribe = function unsubscribe(cb) {
  this.subscribed = false;
  this.sconn.unsubscribe(this.getPrefix(), cb);
};
  ...
  async.waterfall([
    ...
    function addToProcessing(worker, cb) {
      ...
      self.conn.multi()
        ...
        .hset(self.getPrefix('jobs:' + worker.job.id), 'lastUpdate', worker.job.lastUpdate)
